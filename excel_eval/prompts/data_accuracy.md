...
- **Formula errors (#VALUE!, #REF!) are primarily a Formula & Logic issue**: If cells display error values due to broken formulas, penalize this in DA only if the errors affect key output values or metrics that users rely on. Do NOT penalize DA for formula errors in ancillary cells, helper columns, or validation fields — those belong in **Formula & Logic**.
...
- **Source data may contain errors — do NOT blindly penalize corrections**: The scan report treats source data as the reference baseline, but the source data itself may contain factual errors (e.g., wrong drug names, incorrect formulas, mislabeled categories). When the scan report flags a mismatch, consider the possibility that the **generated value might actually be correct** and the source was wrong. If you can verify the AI corrected a genuine factual error, tag it as `+VERIFIED: AI correctly fixed source error — [details]`. If you cannot determine which side is correct, tag it as `UNCONFIRMED` rather than automatically penalizing.
- **Significantly expanded output — be cautious with mismatch penalties**: When the generated file has significantly more data than the source (e.g., source has 9 rows but output has 46), the AI clearly added substantial domain knowledge beyond the source template. In such cases, the few values that differ from the source may represent the AI applying its own knowledge (corrections, updates, domain expertise) rather than making errors. Tag such mismatches as `UNCONFIRMED` and do NOT treat them as confirmed errors unless you have strong independent evidence the generated value is wrong.
...
- **No source data available (web research / creative tasks)**: When no source data or scan comparison is available (e.g., the task requires web research, creative generation, or curating information from external sources), focus on: (a) internal consistency of values within the workbook, (b) obvious factual errors you can independently verify, (c) duplicate entries or self-contradictions. Do NOT penalize data simply because you cannot verify it against a source file — **unverifiable ≠ inaccurate**. Score 2-3 if the data is internally consistent and plausible; only score 0-1 if you find concrete evidence of fabrication or self-contradiction.
- **When scan report says "no comparison possible"**: If the scan report explicitly states it cannot perform a structured comparison, you MUST NOT claim specific value mismatches between source and generated data. You have no cell-level evidence to support such claims, and making them violates the anti-hallucination rules below. Focus only on internal consistency and values you can verify from the generated data alone. Also, "missing data from other time periods or categories" is a **Completeness** issue, not a Data Accuracy error — do NOT penalize DA for content that was never generated.
