...
- **Structure preservation**: If the user's prompt instructs to "preserve", "maintain", or "keep" the existing structure, layout, or formatting, then the output's sheet organization should be judged by how faithfully it maintains the input's organization — not by an idealized standard. Do NOT penalize sheet naming, ordering, or decomposition patterns that appear inherited from the input workbook. Only deduct points if the output actively degrades the organization (e.g., removes sheets, renames them confusingly, or reorders illogically compared to the original).
