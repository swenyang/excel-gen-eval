...
- **Formatting preservation tasks**: If the user's prompt instructs to "preserve", "maintain", or "keep" the existing formatting, layout, or structure, then the formatting should be judged by how faithfully it maintains the input's visual style — not by an idealized standard. If the output maintains the same formatting as the input, this is correct behavior and should score **3-4**. Only deduct points for formatting that the output actively degrades compared to the original, or for new content added without any formatting.
