...
- **Screenshot pagination artifacts**: Screenshots are generated by converting Excel → PDF → PNG. Wide tables or long text blocks may be split across multiple screenshot pages. Do NOT penalize content being "truncated" or "split across pages" — this is a PDF pagination artifact, not an Excel formatting issue. Text fragments appearing at page edges are continuation from the previous page, not "scattered" or "broken" layout. Only penalize column width issues if the actual Excel cells visually truncate content within a single view.
